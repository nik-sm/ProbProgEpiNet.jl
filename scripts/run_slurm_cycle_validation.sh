...
ENTRYPOINT_SCRIPT=$PROJECT_DIR/scripts/worker_startup.cycle_validation.sh
...
## SCENARIOS:
# constant betaE with:
# lo = 0.02 (logit space: -3.892)
# med = 0.15 (logit space: -1.735)
# hi = 0.50 (logit space: 0.0)

# all lo, and a hi peak for a single betaE with patterns:
# [ . ^ . . . .]
# [ . . . ^ . .]
# [ . . . . . ^]
...
seed=1
config=config.json
num_traj=1  # Note that we start with 1 during generation
            # and this is increased to 100 during generate.jl
for json_setting in lo med hi early-peak mid-peak late-peak; do
for county in miamidade-exp middlesex-exp losangeles-exp; do
for iters in 40; do
for prior_gamma_logit_mean in -1.79; do  # 7 days
for prior_lambda_logit_mean in -2.56; do  # 13.9 days
for lead_in_time in 7; do
for inf_thresh_after_lead in 0.01 0.02; do
for lr in 6e-4; do
for knots in 6; do
...
for noise in 25e-5 5e-4; do

    timestamp=$(date -u +"%Y-%m-%dT%H:%M:%S")

    fixed_total_E0=$inf_thresh_after_lead

    output_dir=$output_dir_base/${county}/${json_setting}
    mkdir -p $output_dir
    
    outfile=$LOG_DIR/generate_log.time\=$timestamp,git\=$git_hash

    json_input=validation_cycle/vary_beta/${county}/${json_setting}/generation_params.json

    cmd_string="/bin/bash $ENTRYPOINT_SCRIPT $PROJECT_DIR $@ \
        --json_results=$json_input \
        --obs_noise_std=$noise \
        --iterations=$iters \
        --random_seed=$seed \
        --timestamp=$timestamp \
